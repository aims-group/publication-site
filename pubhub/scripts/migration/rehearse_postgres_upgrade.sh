...
for command_name in docker diff sort mktemp; do
...
cleanup() {
    ...
    docker rm -f "${PG12_CONTAINER}" "${PG16_CONTAINER}" >/dev/null 2>&1 || true
    docker network rm "${NETWORK}" >/dev/null 2>&1 || true
    docker volume rm "${PG12_VOLUME}" "${PG16_VOLUME}" >/dev/null 2>&1 || true
...
wait_for_postgres() {
    local container="$1"
    local attempts=60
    until docker exec "${container}" pg_isready -U postgres -d pubhub >/dev/null 2>&1; do
        attempts=$((attempts - 1))
...
capture_counts() {
    local container="$1"
    local output="$2"
    docker exec -i "${container}" psql -X -Aqt -v ON_ERROR_STOP=1 -U postgres -d pubhub > "${output}" <<'SQL'
SELECT format(
...
docker volume create "${PG12_VOLUME}" >/dev/null
docker volume create "${PG16_VOLUME}" >/dev/null

docker run -d --name "${PG12_CONTAINER}" --network "${NETWORK}" \
    -e POSTGRES_PASSWORD="${PASSWORD}" -e POSTGRES_DB=pubhub \
    -v "${PG12_VOLUME}:/var/lib/postgresql/data" postgres:12.6 >/dev/null
wait_for_postgres "${PG12_CONTAINER}"

echo "Restoring the source backup into PostgreSQL 12..."
case "${SOURCE_BACKUP}" in
    *.sql.gz)
        gzip -dc "${SOURCE_BACKUP}" | docker exec -i "${PG12_CONTAINER}" psql -X -q -v ON_ERROR_STOP=1 -U postgres -d pubhub >/dev/null
        ;;
    *.dump)
        docker cp "${SOURCE_BACKUP}" "${PG12_CONTAINER}:/tmp/source.dump"
        docker exec "${PG12_CONTAINER}" pg_restore --exit-on-error -U postgres -d pubhub /tmp/source.dump
        ;;
    *)
...
docker exec "${PG12_CONTAINER}" pg_dump -U postgres -d pubhub --format=custom --no-owner --no-acl --file=/tmp/pg12.dump
docker cp "${PG12_CONTAINER}:/tmp/pg12.dump" "${TEMP_DIR}/pg12.dump" >/dev/null

docker run -d --name "${PG16_CONTAINER}" --network "${NETWORK}" \
    -e POSTGRES_PASSWORD="${PASSWORD}" -e POSTGRES_DB=pubhub \
    -v "${PG16_VOLUME}:/var/lib/postgresql/data" postgres:16.3 >/dev/null
wait_for_postgres "${PG16_CONTAINER}"

echo "Restoring the PostgreSQL 12 dump into PostgreSQL 16..."
docker cp "${TEMP_DIR}/pg12.dump" "${PG16_CONTAINER}:/tmp/pg12.dump"
docker exec "${PG16_CONTAINER}" pg_restore --exit-on-error -U postgres -d pubhub --no-owner --no-acl /tmp/pg12.dump
docker exec "${PG16_CONTAINER}" vacuumdb -U postgres -d pubhub --analyze-in-stages >/dev/null
capture_counts "${PG16_CONTAINER}" "${TEMP_DIR}/pg16-counts.tsv"

diff -u "${TEMP_DIR}/pg12-counts.tsv" "${TEMP_DIR}/pg16-counts.tsv"

INVALID_INDEXES="$(docker exec "${PG16_CONTAINER}" psql -X -Aqt -U postgres -d pubhub -c 'select count(*) from pg_index where not indisvalid or not indisready;')"
UNVALIDATED_CONSTRAINTS="$(docker exec "${PG16_CONTAINER}" psql -X -Aqt -U postgres -d pubhub -c 'select count(*) from pg_constraint where not convalidated;')"
...
if [[ -n "${MIGRATION_DJANGO_IMAGE:-}" ]]; then
    if ! docker image inspect "${MIGRATION_DJANGO_IMAGE}" >/dev/null 2>&1; then
